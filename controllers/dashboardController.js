...
const markAttendance = async (req, res) => {
    try {
        ...
        // 1. Validate Time (9:30 AM to 5:00 PM IST)
        ...
        const currentTimeMins = istHours * 60 + istMinutes;

        const isTimeValid = (currentTimeMins >= (9 * 60 + 30) && currentTimeMins <= (17 * 60 + 0));
        ...
        // 3. Dynamic Geofence Validation (500 meters)
        let isWithinGeofence = false;
        const studentBranchKey = Object.keys(BRANCH_LOCATIONS).find(
            b => b.toLowerCase() === (branch || "").trim().toLowerCase()
        );
        const branchCoords = studentBranchKey ? BRANCH_LOCATIONS[studentBranchKey] : BRANCH_LOCATIONS["Bangalore"];
        ...
        if (!isTimeValid) return res.status(400).json({ success: false, message: "Attendance allowed only between 9:30 AM and 5:00 PM." });
        if (!isWithinGeofence) return res.status(400).json({ success: false, message: `Location Restriction: Must be within 500m of your registered ${branch || 'campus'} branch.` });
